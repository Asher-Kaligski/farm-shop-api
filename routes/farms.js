...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (!user.roles.includes('FARM_OWNER')) return res.status(403).send('The user doesn\'t have permission');

    let farm = await Farm.findOne({name: req.body.name});
    if(farm) return res.status(400).send('The farm with given name already exists');
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const user = await User.findById(req.body.farmOwnerId);
    if (!user) return res.status(404).send('The farm owner with given ID has not been found');
    if (!user.roles.includes('FARM_OWNER')) return res.status(403).send('The user doesn\'t have permission');
